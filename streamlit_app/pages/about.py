    ...
    **Song Predictor** is an ML-powered tool designed to predict which songs are likely to be chosen as favorites.
    ...
    with c1:
        with st.container(border=True, height="stretch"):
            ...
            st.markdown("Add songs from Spotify URLs or pull random examples from history.")
    ...
    with c4:
        with st.container(border=True, height="stretch"):
            ...
            st.markdown("Run the model to see if the track ranks as a **Weekly Winner** or is likely to be a **Favorite**.")
    ...
    with m1:
        with st.container(border=True, height="stretch"):
            ...
            st.write("Simulates a competitive week. It compares all drafted candidates against one another and highlights the single track with the highest probability.")
    
    with m2:
        with st.container(border=True, height="stretch"):
            st.markdown("#### 🔎 Check Selected")
            st.markdown("**Question:** *\"Is this song good enough?\"*")
            st.write("Analyzes a single song in isolation. It checks if the song's probability score crosses the model's confidence threshold, regardless of other tracks.")
